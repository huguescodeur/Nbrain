...
def register(request):
    if request.method == "POST":
        ...
        try:
            ...
            user.cover = "profile_pic/no_pic.png"
...
#     followings, suggestions, follower = [], [], False
#     if request.user.is_authenticated:
#         followings = Follower.objects.filter(followers=request.user).values_list('user', flat=True)
#         suggestions = User.objects.exclude(pk__in=followings).exclude(username=request.user.username).order_by("?")[:6]
#         follower = request.user in Follower.objects.get(user=user).followers.all()
...
def profile(request, username):
    user = get_object_or_404(User, username=username)
    all_posts = Post.objects.filter(creater=user).order_by('-date_created')
    recent_posts = Post.objects.filter(creater=user).order_by('-date_created')[:3]
    ...
    return render(request, 'network/profile.html', {
        "username": user,
        'recent_posts': recent_posts,
        "posts_count": all_posts.count(),  # Ajout du nombre de publications
        "suggestions": suggestions,
        "page": "profile",
        "is_follower": follower,
        "follower_count": follower_count,
...
def followers(request):
    if request.user.is_authenticated:
        ...
        print(f"following_user: {following_user}")
