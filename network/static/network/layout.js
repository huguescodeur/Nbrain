...
function login_popup(action) {
  ...
  if (action === "like") {
    ...
    document.querySelector(".main_text-div").querySelector("h2").innerText =
      "Aimez une publication pour montrer votre soutien";
    ...
    document.querySelector(".main_text-div").querySelector("h2").innerText =
      "Commentez pour participer à la conversation";
  } else if (action === "save") {
    document.querySelector(".icon-div").innerHTML = `
        <svg width="2.5em" height="2.5em" viewBox="0 0 16 16" class="bi bi-bookmark-fill" fill="#6366F1" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" d="M3 3a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v12l-5-3-5 3V3z"/>
        </svg>`;
    document.querySelector(".main_text-div").querySelector("h2").innerText =
      "Enregistrer la publication pour revoir plus tard";
  } else if (action === "follow") {
    document.querySelector(".icon-div").innerHTML = `
        <svg width="2.5em" height="2.5em" viewBox="0 0 24 24" fill="#6366F1" class="r-1re7ezh r-4qtqp9 r-yyyyoo r-1q142lx r-1xvli5t r-19einr3 r-dnmrzs r-bnwqim r-1plcrui r-lrvibr">
...
function follow_user(element, username, origin) {
  ...
  fetch("/" + username + "/follow", {
    ...
    if (origin === "suggestion") {
      element.parentElement.innerHTML = `<button style="padding: 10px 0 10px; border: 2px solid #6366F1" class="transition ease-in-out px-4 me-2 text-sm  font-bold text-white focus:outline-none  rounded-full bg-indigo-500  hover:border-white-100 hover:text-white hover:bg-indigo-400 bg-white-100 flex items-center justify-center" type="button" onclick="unfollow_user(this,'${username}','suggestion')">Suivi(e)</button>`;
    } else if (origin === "edit_page") {
      element.parentElement.innerHTML = `<button style="padding: 10px 0 10px; border: 2px solid #6366F1" class="transition ease-in-out px-4 me-2 text-sm  font-bold text-white focus:outline-none  rounded-full bg-indigo-500  hover:border-white-100 hover:text-white hover:bg-indigo-400 bg-white-100 flex items-center justify-center float-right" onclick="unfollow_user(this,'${username}','edit_page')" id="following-btn">Suivi(e)</button>`;
...
function unfollow_user(element, username, origin) {
  ...
  fetch("/" + username + "/unfollow", {
    ...
    if (origin === "suggestion") {
      element.parentElement.innerHTML = `<button style="padding: 10px 0 10px; border: 2px solid #6366F1" class="transition ease-in-out px-4 me-2 text-sm  font-bold text-gray-900 focus:outline-none  rounded-full border-2 border-indigo-500  hover:border-white-100 hover:text-white hover:bg-indigo-400 bg-white-100 flex items-center justify-center" type="button" onclick="follow_user(this,'${username}','suggestion')">Suivre</button>`;
    } else if (origin === "edit_page") {
      element.parentElement.innerHTML = `<button style="padding: 10px 0 10px; border: 2px solid #6366F1" class="transition ease-in-out px-4 me-2 text-sm  font-bold text-gray-900 focus:outline-none  rounded-full border-2 border-indigo-500  hover:border-white-100 hover:text-white hover:bg-indigo-400 bg-white-100 flex items-center justify-center float-right" onclick="follow_user(this,'${username}','edit_page')" id="follow-btn">Suivre</button>`;
    } else if (origin === "dropdown") {
      ///////////////////////////////////////////////////////////////////////////////////////////
    }
